...
% 下面这组数据力矩坐标系变换有问题，此外考虑了弦向阻尼力矩——不正确——但是模式搜索之后的结果与实验匹配不错
...
% M_Iz_inertia=Iz_small.*domega_z*10^9;      % uW ——这里翅平面坐标系下Iz_inertia.*domega_z, 需要*cos(psi)变换到惯性坐标下下
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure(21) 
...
% M_ztotal=M_ztrans+M_zrot+M_zadd+M_inert_z;  % 翅平面: 弦向(z轴)  %——不应该考虑z轴方向的阻尼力矩
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure(32)
% hold on
...
%% 第一部分——虫体坐标系下的X轴方向力矩
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% M_X=cos(phi).*M_xtotal+sin(phi).*cos(psi).*M_ytotal-sin(phi).*sin(psi).*M_ztotal;  % 早期采用该公式与实验数据结果进行对比
M_X=cos(phi).*M_xtotal-sin(phi).*cos(psi).*M_ytotal-sin(phi).*sin(psi).*M_ztotal;  % 早期采用该公式与实验数据结果进行对比
% M_X=cos(phi).*M_xtotal-sin(phi).*sin(psi).*M_ztotal;      % 俯仰轴Z——俯仰力矩
% M_X=cos(phi).*M_xtotal-sin(phi).*cos(psi).*M_ytotal+sin(phi).*sin(psi).*M_ztotal;      % 俯仰轴X——俯仰力矩
...
M_Y=sin(phi).*M_xtotal+cos(phi).*cos(psi).*M_ytotal-cos(phi).*sin(psi).*M_ztotal;  % 翻滚轴Y——翻滚力矩
% figure(36)
...
M_Z=sin(psi).*M_ytotal+cos(psi).*M_ztotal;  % 偏航轴Z——偏航力矩
...
% pitchmoment_science=xlsread('D:\KXJ\PassiveRot_dynamic_Science_fruitfly\aerodynamic_moment\aerodynamic_moment_model2_2_MCoeff_optimal\aerodynamic_moment_model2_2_MCoeff_optimal\force_pitch_y_moment_ForceModul.xlsx','A1:D1145'); % 读入数据-早期的数据不是稳态翅拍的数据
...
My_norm1=pitchmoment_science(:,3);     % 俯仰力矩——原始数据输出时+0.5
Mz_norm1=pitchmoment_science(:,4);      % 偏航力矩
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
t_NOfreq=[t_NOfreq1+0.0052824335;t_NOfreq1+T+0.0052824335;t_NOfreq1+2*T+0.0052824335];
Mx_norm=[My_norm1;My_norm1;My_norm1];          % 俯仰轴X——俯仰力矩
My_norm=[Mx_norm1;Mx_norm1;Mx_norm1];          % 翻滚轴Y——翻滚力矩
Mz_norm=[Mz_norm1;Mz_norm1;Mz_norm1];          % 偏航轴Z——偏航力矩
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% title('虫体坐标下,准稳态理论预测的俯仰力矩和实验测得的冲程角引起的翻滚力矩的对比')   
% grid on
box on
% set(gca,'XTick',(0.9:0.1:4.05))
set(gca,'LineStyle','-','LineWidth',1.5,'FontSize',20,'FontName','Times','FontWeight','Bold') 
...
% title('虫体坐标下,准稳态理论预测的俯仰力矩和实验测得的冲程角引起的偏航力矩的对比')   
% grid on
box on
% set(gca,'XTick',(0.9:0.1:4.05))
set(gca,'LineStyle','-','LineWidth',1.5,'FontSize',20,'FontName','Times','FontWeight','Bold') 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% stdev_pitch=std(((2*M_X/norm_denom)-(Mx_norm-0.02)),0,1);   % 按照S1(0)求解各列(1)元素的标准方差
stdev_pitch=std(((2*M_X/norm_denom)-Mx_norm),0,1);   % 按照S1(0)求解各列(1)元素的标准方差
